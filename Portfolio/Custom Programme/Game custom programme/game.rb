require 'gosu'
require 'rubygems'
require_relative 'player1'
require_relative 'enemy'
require_relative 'bullet'
require_relative 'credit'
require_relative 'explosion'
require_relative 'player2'

module ZOrder
  HIDDEN, BACKGROUND, SCENE, UI, TOP = *0..4
end
#
class MyGame < Gosu::Window
#---- size of window ----
  HEIGHT = 800
  WIDTH = 1500
  FREQUENCY = 0.1
 def initialize()
  super(WIDTH,HEIGHT,false)
  self.caption = "Spaceship Hunter"
  @player = Player.new(self)

  #--------------- start point --------------------
  @scene = :start
  @start_music = Gosu::Song.new('musics/start.wav')
  @start_background = Gosu::Image.new('images/start.jpg')
  @title = Gosu::Font.new(50)
  @start_options = Gosu::Font.new(35)
  @info_font = Gosu::Font.new(15)
  @hover_options_1 = ZOrder::HIDDEN; @hover_options_2 = ZOrder::HIDDEN; @hover_options_3 = ZOrder::HIDDEN
  @error_music = Gosu::Sample.new('musics/error.wav')
  @start_play_music = Gosu::Sample.new('musics/start_play.wav')
  @point = Gosu::Image.new('images/pointing.png')
  #-------------- instruction -------------------------
  @instruction_background = Gosu::Image.new('images/instruction.jpg')
  @instruction_music = Gosu::Song.new('musics/instruction.wav')
  @description = Gosu::Font.new(30)
  #--------------- selecting -------------------------
  @selecting_music = Gosu::Song.new('musics/selecting.wav')
  @select_char_1 = ZOrder::HIDDEN; @select_char_2 = ZOrder::HIDDEN
  @select_color = 0xFF555555
  @ship1_sel = Gosu::Image.new('images/ship1_select.png')
  @ship2_sel = Gosu::Image.new('images/ship2_select.png')




  #-----------------playing ---------------------------
  @playing_music = Gosu::Song.new('musics/playing.wav')
  @counting = Gosu::Font.new(45)
  @shooting_music = Gosu::Sample.new('musics/shooting.wav')
  @die_music = Gosu::Song.new('musics/die.wav')
  #-------------- end --------------------------
  @end_music = Gosu::Song.new('musics/end.wav')
  @end_message_font = Gosu::Font.new(15)

 end

 def draw
  case @scene
    when :start
      draw_start
    when :instruction
      draw_instruction
    when :selecting
      draw_selecting
    when :playing
      draw_playing
    when :end
      draw_end
    end
  end

  def draw_start
    @start_background.draw(0, 0, z = ZOrder::BACKGROUND)
    @start_music.play(false)
    @title.draw_text("Spaceship hunter", 650, 50, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)
    @start_options.draw_text("Play \n\n\nInstructions \n\n\nExit
      ", 500, 300, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)

    @point.draw(355,305,1, @hover_options_1)
    @point.draw(355,410,1, @hover_options_2)
    @point.draw(355,510,1, @hover_options_3)

    draw_quad(490,295, Gosu::Color::BLACK, 750, 295, Gosu::Color::BLACK, 750, 340, Gosu::Color::BLACK, 490, 340, Gosu::Color::BLACK, @hover_options_1)
    draw_quad(490,400, Gosu::Color::BLACK, 750, 400, Gosu::Color::BLACK, 750, 445, Gosu::Color::BLACK, 490, 445, Gosu::Color::BLACK, @hover_options_2)
    draw_quad(490,505, Gosu::Color::BLACK, 750, 505, Gosu::Color::BLACK, 750, 550, Gosu::Color::BLACK, 490, 550, Gosu::Color::BLACK, @hover_options_3)

  end

  def draw_instruction
    @instruction_background.draw(0,0, ZOrder::BACKGROUND)
    @title.draw_text("          INSTRUCTION\n Created by Thanh Minh \n\n How to Play", 500, 50, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)
    @description.draw_text("Use keyboard to control:\n- ↑ || W to move up\n- ↓ || S to move down\n- → || D to move right\n- ← || A to move left\n- Space to Attack\n\n*REMEMBER: To avoid the enemy\n\n**There are many many enemy's illusion so WATCH OUT\nYou cannot destroy it but it still can take you down
      \nPress Q || Space to be back to the Menu",500,300, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)
  end

  def draw_selecting
    @selecting_music.play(false)
    @title.draw_text("Selecting Your Player
      ", 550,50, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)

      draw_quad(0,0, Gosu::Color::BLACK, WIDTH, 0, Gosu::Color::BLACK, WIDTH, HEIGHT, Gosu::Color::BLACK, 0, HEIGHT, Gosu::Color::BLACK, ZOrder::BACKGROUND)
      draw_quad(0,0, @select_color, WIDTH/2, 0, @select_color, WIDTH/2, 1500, @select_color, 0, 1500, @select_color, @select_char_1)
      draw_quad(WIDTH/2,0, @select_color, WIDTH, 0, @select_color, WIDTH, HEIGHT, @select_color, WIDTH/2, HEIGHT, @select_color, @select_char_2)

      @title.draw_text("Chaos Eater
        ", 250,300, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)
      @title.draw_text("Ship Eater
        ", 1050,300, z = ZOrder::TOP, 1, 1, Gosu::Color::WHITE)
      @player.draw_select

    end

  def draw_playing
    @ship.draw
    @enemies.each do |enemy|
      enemy.draw
    end
    @bullets.each do |bullet|
      bullet.draw
    end
    @explosions.each do |explosion|
      explosion.draw
    end
    @counting.draw_text("Score:",30,10,1)
    @counting.draw_text(@count,150,10,1)

  end

  def draw_end
    clip_to(60, 140, 1400, 600) do
      @credits.each do |credit|
          credit.draw
      end
    end
    @end_message_font.draw_text(@end_message1, 400, 20, 1, 4, ZOrder::TOP, Gosu::Color::WHITE)
    @end_message_font.draw_text(@end_message2, 550, 60, 1, 4, ZOrder::TOP, Gosu::Color::WHITE)
    @end_message_font.draw_text(@bottom_end, 250, HEIGHT - 55, 1, 3, ZOrder::TOP, Gosu::Color::WHITE)
  end

  def initialize_menu
    case @scene
    when :start
      initialize_start
    when :instruction
      initialize_instruction
    when :selecting
      initialize_selecting
    when :playing
      initialize_playing
    when :end
      initialize_end
    end
  end

  def initialize_start
    @scene = :start

  end

  def initialize_instruction
    sleep(0.5)
    @scene = :instruction
    @instruction_music.play(false)
  end

  def initialize_selecting
    @scene = :selecting
  end

  def initialize_playing
    @scene = :playing
    @enemies = []
    @bullets = []
    @explosions = []
    @credits =[]
    @count = 0
    @playing_music.play(false)

  end

  def initialize_end(fate)
    @scene = :end
    @end_music.play
    sleep(0.2)
    case fate
    when :die
      @end_message1 = "You was destroyed by enemy ship"
    when :escape
      @end_message1 = "You have exited the game"
    end
    @end_message2 = "Your score: #{@count}"
    @bottom_end = "Press B to back to Menu, Press Q or Escape to exit"
    y = 1300
    File.open('credits.txt').each do |line|
      @credits.push(Credit.new(self, line.chomp, 100, y))
      y += 30
    end
  end

  def update
    case @scene
    when :start
      update_start
    when :instruction
      update_instruction
    when :selecting
      update_selecting
    when :playing
      update_playing
    when :end
      update_end
    end
  end

  def update_start
    if (mouse_x > 489 && mouse_x < 756 && mouse_y > 294 && mouse_y < 341) || ((button_down?(Gosu::KbUp) || button_down?(Gosu::KbW))&& @hover_options_2 == ZOrder::UI)
      @hover_options_1 = ZOrder::UI
      @hover_options_2 = ZOrder::HIDDEN
      @hover_options_3 = ZOrder::HIDDEN
    elsif (mouse_x > 489 && mouse_x < 756 && mouse_y > 399 && mouse_y < 446) || ((button_down?(Gosu::KbUp) || button_down?(Gosu::KbW)) && @hover_options_3 == ZOrder::UI) || ((button_down?(Gosu::KbDown) || button_down?(Gosu::KbS)) && @hover_options_1 == ZOrder::UI)
      @hover_options_1 = ZOrder::HIDDEN
      @hover_options_2 = ZOrder::UI
      @hover_options_3 = ZOrder::HIDDEN
      sleep(0.15)
    elsif (mouse_x > 489 && mouse_x < 756 && mouse_y > 504 && mouse_y < 556) || ((button_down?(Gosu::KbDown) || button_down?(Gosu::KbS)) && @hover_options_2 == ZOrder::UI)
      @hover_options_1 = ZOrder::HIDDEN
      @hover_options_2 = ZOrder::HIDDEN
      @hover_options_3 = ZOrder::UI
    elsif ((button_down?(Gosu::KbDown) || button_down?(Gosu::KbS)) && @hover_options_1 == ZOrder::HIDDEN && @hover_options_2 == ZOrder::HIDDEN && @hover_options_3 == ZOrder::HIDDEN)
      @hover_options_1 = ZOrder::UI
      @hover_options_2 = ZOrder::HIDDEN
      @hover_options_3 = ZOrder::HIDDEN
    elsif ((button_down?(Gosu::KbUp) || button_down?(Gosu::KbW))&& @hover_options_1 == ZOrder::HIDDEN && @hover_options_2 == ZOrder::HIDDEN && @hover_options_3 == ZOrder::HIDDEN)
      @hover_options_1 = ZOrder::HIDDEN
      @hover_options_2 = ZOrder::HIDDEN
      @hover_options_3 = ZOrder::UI
    end

    if (button_down?(Gosu::KbUp) && @hover_options_1 == ZOrder::UI) || (button_down?(Gosu::KbDown) && @hover_options_3 == ZOrder::UI) || button_down?(Gosu::KbLeft) || button_down?(Gosu::KbRight)
      @error_music.play
      sleep(0.5)
    end
  end

  def update_instruction

  end

  def update_selecting
    if mouse_x > 0 && (mouse_x < WIDTH/2) && mouse_y > 0 && mouse_y < 800
      @select_char_1 = ZOrder::BACKGROUND
      @select_char_2 = ZOrder::HIDDEN
    elsif (mouse_x > WIDTH/2) && mouse_x < WIDTH && mouse_y > 0 && mouse_y < 800
      @select_char_1 = ZOrder::HIDDEN
      @select_char_2 = ZOrder::BACKGROUND
    else
      @select_char_1 = ZOrder::HIDDEN
      @select_char_2 = ZOrder::HIDDEN
    end

  end

  def update_playing
    @ship.go_up if button_down?(Gosu::KbUp) || button_down?(Gosu::KbW)
    @ship.go_down if button_down?(Gosu::KbDown) || button_down?(Gosu::KbS)
    @ship.go_right if button_down?(Gosu::KbRight) || button_down?(Gosu::KbD)
    @ship.go_left if button_down?(Gosu::KbLeft) || button_down?(Gosu::KbA)
    #----------------enemy -------------------------
    if rand < FREQUENCY
      @enemies.push Enemy.new(self)
    end
    @enemies.each do |enemy|
      enemy.move
    end
    #---------------delete enemy -------------------
    @enemies.dup.each do |enemy|
      if enemy.y > HEIGHT + enemy.edge
        @enemies.delete enemy
      end
    end
    #-------------remove bullet no longer on screen-----------
    @bullets.dup.each do |bullet|
      @bullets.delete bullet unless bullet.onscreen?
    end
    @bullets.each do |bullet|
      bullet.move
    end
    #--------------- explosion -----------------
    @explosions.dup.each do |explosion|
      @explosions.delete explosion if explosion.done
    end
    #--------------- when bullet hit enemy -------------------------
    @enemies.dup.each do |enemy|
      @bullets.dup.each do |bullet|
        distance = Gosu.distance(enemy.x, enemy.y, bullet.x, bullet.y)
        if distance < bullet.radius + enemy.edge
          @enemies.delete enemy
          @bullets.delete bullet
          @count += 1
          @explosions.push Explosion.new(self, enemy.x,enemy.y)
        end
      end
    end
    #---------------- when player hit enemy ----------------------
    @enemies.each do |enemy|
      distance = Gosu.distance(enemy.x,enemy.y, @ship.x, @ship.y)
      initialize_end(:die) if distance < @ship.rotate + enemy.edge
    end
      initialize_end(:escape) if button_down?(Gosu::KbEscape)
  end

  def update_end
    @credits.each do |credit|
      credit.move
    end
    if @credits.last.y < 100
        @credits.each do |credit|
            credit.reset
        end
    end
  end

  def button_down(id)
    case @scene
    when :start
      button_down_start(id)
    when :instruction
      button_down_instruction(id)
    when :selecting
      button_down_selecting(id)
    when :playing
      button_down_playing(id)
    when :end
      button_down_end(id)
    end
  end


  def button_down_start(id)
    if (id == Gosu::MsLeft || id == Gosu::KbReturn) && @hover_options_1 == ZOrder::UI
      initialize_selecting
      @start_play_music.play
      sleep(0.2)
    elsif (button_down?(Gosu::MsLeft) || button_down?(Gosu::KbReturn)) && @hover_options_2 == ZOrder::UI
      initialize_instruction
    elsif (button_down?(Gosu::MsLeft) || button_down?(Gosu::KbReturn)) && @hover_options_3 == ZOrder::UI
      close

    end

  end

  def button_down_instruction(id)
    if id == Gosu::KbQ || id == Gosu::KbSpace
      initialize_start
    end
  end

  def button_down_selecting(id)
    if (id == Gosu::KbReturn || id == Gosu::MsLeft) && mouse_x > 0 && mouse_x < WIDTH/2 && mouse_y > 0 && mouse_y < HEIGHT && @select_char_1 == ZOrder::BACKGROUND
      initialize_playing
      @ship1_sel = Ship1.new(self)
      @ship = @ship1_sel
    elsif (id == Gosu::KbReturn || id == Gosu::MsLeft) && mouse_x > WIDTH/2 && mouse_x < WIDTH && mouse_y > 0 && mouse_y < HEIGHT && @select_char_2 == ZOrder::BACKGROUND
      initialize_playing
      @ship2_sel = Ship2.new(self)
      @ship = @ship2_sel
    elsif id == Gosu::KbQ || id == Gosu::KbEscape
      initialize_start
    end
  end

  def button_down_playing(id)
    if id == Gosu::KbSpace || id == Gosu::MsLeft || id == Gosu::KbReturn
        @bullets.push Bullet.new(self, @ship.x, @ship.y, @ship.edge)
        @shooting_music.play(0.3)
    end
  end

  def button_down_end(id)
    if id  == Gosu::KbP
      initialize_selecting
    elsif id == Gosu::KbB
      initialize_start
    elsif id == Gosu::KbQ || id == Gosu::KbEscape
      close
    end
  end

end



window = MyGame.new()
window.show()



